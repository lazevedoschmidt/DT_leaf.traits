...
mf <- lmerTest::lmer(perc.mine_scale ~ CN_scale + LMA_scale + wt.perc_N_scale +wt.perc_C_scale + 
            perc.P_scale + (1|Forest), data = widedata)
summary(mf) #singular

md <- lmerTest::lmer(div.mine_scale ~ CN_scale + LMA_scale + wt.perc_N_scale +wt.perc_C_scale + 
            perc.P_scale + (1|Forest), data = widedata)
summary(md) #sig and singular
...
#           show.intercept = FALSE,
#           file = "alltab.div.html")
# tab_model(area.all,
#           dv.labels = "% Area Damaged",
#           show.intercept = FALSE,
#           file = "all.areaperc.html")

#HF specific models----
tf.hf <- glm(perc.dam_logit ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit + trichomes_scale , data = newtraitHF)
summary(tf.hf) 
AIC(tf.hf) #13.3
#Best model, not singular, and secondary compounds do not improve

td.hf <- glm(div.dam_scale ~ CN_scale * LMA_scale+ wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit , data = newtraitHF)
summary(td.hf) 
AIC(td.hf) #29.1
#Best model, not singular, and secondary compounds do not improve

sf.hf <- glm(perc.spec_logit ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit + trichomes_scale, data = newtraitHF)
summary(sf.hf) 
AIC(sf.hf) #22.3
#Best model, not singular, and secondary compounds do not improve 

sd.hf <- glm(div.spec_scale ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit , data = newtraitHF)
summary(sd.hf) 
AIC(sd.hf) #26.1
#Best model, not singular, and secondary compounds do not improve

gf.hf <- glm(perc.gall_logit ~ CN_scale + LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit , data = newtraitHF)
summary(gf.hf) #sig
AIC(gf.hf) #-26.1
#Best model, not singular, and secondary compounds do not improve

gd.hf <- glm(div.gall_scale ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit + trichomes_scale, data = newtraitHF)
summary(gd.hf) #almost sig
AIC(gd.hf) #15.7
#Best model, not singular, and secondary compounds do not improve

mf.hf <- glm(perc.mine_logit ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit + lignin.perc_logit, data = newtraitHF)
summary(mf.hf) 
AIC(mf.hf) #6.8
#Best model, not singular, and lignin improves this model! 

md.hf <- glm(div.mine_scale ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit + trichomes_scale, data = newtraitHF)
...
#HF area damaged----
area.hf <- glm(perc.area.dam..mean._logit ~ CN_scale * LMA_scale + wt.perc_N_logit *wt.perc_C_logit + 
                 perc.P_logit + trichomes_scale, data = newtraitHF)
...
tab_model(area.hf,
          dv.labels = "% Area Damaged",
          show.intercept = FALSE,
          file = "HF.areaperc.html")

#MD specific models----
tf.md <- glm(perc.dam_logit ~ CN_scale + LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit +  margin_type_scale, data = newtraitMD)
summary(tf.md) 
AIC(tf.md) #-2.8
#Best model, not singular, and secondary compounds do not improve

td.md <- glm(div.dam_scale ~ CN_scale + LMA_scale+ wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit +  margin_type_scale, data = newtraitMD)
summary(td.md) #sig
AIC(td.md) #12.7
#Best model, not singular, and secondary compounds do not improve

sf.md <- glm(perc.spec_logit ~ CN_scale + LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit +  margin_type_scale, data = newtraitMD)
summary(sf.md) #sig
AIC(sf.md) #12.6
#Best model, not singular, and secondary compounds do not improve

sd.md <- glm(div.spec_scale ~ CN_scale + LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit +  perc.water_logit, data = newtraitMD)
summary(sd.md) #sig
AIC(sd.md) #14.5
#Best model, not singular, and water actually improves the model 

gf.md <- glm(perc.gall_logit ~ CN_scale + LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit +  margin_type_scale, data = newtraitMD)
summary(gf.md) 
AIC(gf.md) #-15.9
#Best model, not singular, and secondary compounds do not improve

gd.md <- glm(div.gall_scale ~ CN_scale + LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit +  margin_type_scale, data = newtraitMD)
summary(gd.md) 
AIC(gd.md) #15.6 
#Best model, not singular, and secondary compounds do not improve

mf.md <- glm(perc.mine_logit ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit +  trichomes_scale, data = newtraitMD)
summary(mf.md)
AIC(mf.md) #2.8
#Best model, not singular, and secondary compounds do not improve

md.md <- glm(div.mine_scale ~ CN_scale + LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               perc.P_logit +  perc.water_logit, data = newtraitMD)
summary(md.md) #sig 
AIC(md.md) #18.7
...
#MD area damaged----
area.md <- glm(perc.area.dam..mean._logit ~ CN_scale+ LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
                 perc.P_logit +  perc.water_logit, data = newtraitMD)
...
tab_model(area.md,
          dv.labels = "% Area Damaged",
          show.intercept = FALSE,
          file = "MD.areaperc.html")

#LS specific models----
#can't use phosphorus because there isn't enough data
tf.ls <- glm(perc.dam_logit ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit +
               trichomes_scale, data = newtraitLS)
summary(tf.ls) 
AIC(tf.ls) #27.8
#Best model, not singular, and secondary compounds do not improve

td.ls <- glm(div.dam_scale ~ CN_scale + LMA_scale+ wt.perc_N_logit +wt.perc_C_logit + 
               trichomes_scale, data = newtraitLS)
summary(td.ls) 
AIC(td.ls) #37.6
#Best model, not singular, and secondary compounds do not improve

sf.ls <- glm(perc.spec_logit ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               trichomes_scale, data = newtraitLS)
summary(sf.ls) 
AIC(sf.ls) #28.7
#Best model, not singular, and secondary compounds do not improve

sd.ls <- glm(div.spec_scale ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               trichomes_scale, data = newtraitLS)
summary(sd.ls)
AIC(sd.ls) #34.2
#Best model, not singular, and secondary compounds do not improve

gf.ls <- glm(perc.gall_logit ~ CN_scale + LMA_scale + wt.perc_N_logit +wt.perc_C_logit, data = newtraitLS)
summary(gf.ls) #sig
AIC(gf.ls) #-16.6
#Best model, not singular, and secondary compounds do not improve

gd.ls <- glm(div.gall_scale ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               trichomes_scale, data = newtraitLS)
summary(gd.ls) #sig
AIC(gd.ls) #33.7
#Best model, not singular, and secondary compounds do not improve

mf.ls <- glm(perc.mine_logit ~ CN_scale + LMA_scale + wt.perc_N_logit +wt.perc_C_logit, data = newtraitLS)
summary(mf.ls) 
AIC(mf.ls) #-25.7
#Best model, not singular, and secondary compounds do not improve

md.ls <- glm(div.mine_scale ~ CN_scale * LMA_scale + wt.perc_N_logit +wt.perc_C_logit + 
               trichomes_scale, data = newtraitLS)
summary(md.ls) #almost sig
AIC(md.ls) #29.3
#Best model, not singular, and secondary compounds do not improve

#LS area damaged----
area.ls <- glm(perc.area.dam..mean._logit ~ CN_scale + LMA_scale + wt.perc_N_logit * wt.perc_C_logit + 
                 trichomes_scale, data = newtraitLS)
summary(area.ls)   
AIC(area.ls)
